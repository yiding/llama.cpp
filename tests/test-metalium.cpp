...
struct test_case {
    test_case(std::function<ggml_tensor *(ggml_context *)>                          build_graph,
              const std::function<double(const float *, const float *, size_t n)> & loss    = nmse,
              double                                                                max_err = 1e-4) :
        max_err(max_err),
        loss(loss),
        build_graph(std::move(build_graph)) {}
    ...
    void eval(ggml_backend_t backend1, ggml_backend_t backend2) {
        ...
        for (ggml_backend_t backend : { backend1, backend2 }) {
            for (ggml_tensor * t = ggml_get_first_tensor(ctx); t != NULL; t = ggml_get_next_tensor(ctx, t)) {
                if (!ggml_backend_supports_op(backend, t)) {
                    GTEST_SKIP() << "not supported by backend: " << ggml_backend_name(backend)
                                 << ". Rejected OP: " << ggml_op_desc(t);
                }
            }
        }
        // post-graph sentinel
        ...
        callback_userdata ud{ max_err, backend1, backend2, loss };
        ...
        auto callback = [](int index, ggml_tensor * t1, ggml_tensor * t2, void * user_data) -> bool {
            ...
            for (size_t i = 0; i < f1.size(); i++) {
                ...
                if (std::isnan(f1[i]) || std::isnan(f2[i])) {
                    ADD_FAILURE() << "[" << ggml_op_desc(t1) << "] NaN at index " << i << " (" << bn1 << "=" << f1[i]
                                  << " " << bn2 << "=" << f2[i] << ")";
                    return true;
                }
                ...
                if (isinf_or_max(f1[i]) || isinf_or_max(f2[i])) {
                    if (isinf_or_max(f1[i]) && isinf_or_max(f2[i])) {
                        if (std::signbit(f1[i]) != std::signbit(f2[i])) {
                            ADD_FAILURE() << "[" << ggml_op_desc(t1) << "] inf sign mismatch: " << bn1 << "=" << f1[i]
                                          << " " << bn2 << "=" << f2[i];
                            return true;
                        }
                    } else {
                        ADD_FAILURE() << "[" << ggml_op_desc(t1) << "] inf mismatch: " << bn1 << "=" << f1[i] << " "
                                      << bn2 << "=" << f2[i];
                        return true;
                    }
                }
...
static std::unique_ptr<test_case> make_test(const std::function<ggml_tensor *(ggml_context *)> & build_graph,
                                            std::string,
                                            float max_err = 1e-4) {
...
class MetaliumTest : public ::testing::Test {
  protected:
    ...
    template <typename... Args> void compare_graph(Args &&... args) {
...
class MetaliumUnaryOpTest : public MetaliumTest,
...
INSTANTIATE_TEST_SUITE_P(
    UnaryOp,
    MetaliumUnaryOpTest,
    ::testing::Combine(::testing::ValuesIn({ GGML_UNARY_OP_ABS, GGML_UNARY_OP_SGN, GGML_UNARY_OP_NEG,
                                             GGML_UNARY_OP_STEP,  // Not supported by Metalium
                                             GGML_UNARY_OP_TANH, GGML_UNARY_OP_ELU, GGML_UNARY_OP_RELU,
    ...
    [](const testing::TestParamInfo<std::tuple<ggml_unary_op, ggml_type>> & info) {
        return std::string(ggml_unary_op_name(std::get<0>(info.param))) + "__" +
               ggml_type_name(std::get<1>(info.param));
    });
...
TEST_F(MetaliumTest, View_flat_buffer_into_3D_tensor_with_offset) {
    compare_graph([](ggml_context * ctx) {
        ggml_tensor * a = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, 32 * 32 * 64);
        ggml_tensor * view =
            ggml_view_3d(ctx, a, 32, 32, 32, 32 * sizeof(float), 32 * 32 * sizeof(float), 32 * sizeof(float));
        ggml_tensor * b = ggml_cont(ctx, view);
        return b;
    });
}
